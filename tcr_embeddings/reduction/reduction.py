...
class JohnsonLindenstarauss:
    ...
    def load_transformation(self):
        self.transformation = pd.read_parquet(
            dir / "jl-transformations" / self._fname
        ).to_numpy()
...
class AutoEncoder:
    def __init__(
        self, encoding_method, method_name: str, out_dim=5, batchsize: int = 1024
    ):
    ...
    def create_transformation(self, dataset_paths):
        try:
            ...
            torch.save(
                ...
                dir / "ae-transformations" / self._encoder_fname,
            )
            torch.save(
                autoencoder.decoder.state_dict(),
                dir / "ae-transformations" / self._decoder_fname,
            )
    ...
    def load_transformation(self):
        ...
        if torch.cuda.is_available():
            encoder.load_state_dict(
                torch.load(dir / "ae-transformations" / self._encoder_fname)
            )
        else:
            encoder.load_state_dict(
                torch.load(
