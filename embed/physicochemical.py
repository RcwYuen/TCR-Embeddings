...
class PhysicoChemicalEncoder(Embedder):
    def __init__(self, fname):
        ...
        self.embedding_tensor = self.embedding_tensor.cuda() if torch.cuda.is_available() else self.embedding_tensor
    ...
    def calc_vector_representations(self, df, *args, **kwargs):
        ...
        rep = torch.zeros((len(tcrs), embedding_tensor.shape[1]), dtype=torch.float32, device=device)

        # Preprocess sequences on the CPU
        tcr_indices = [
            torch.tensor([self.amino_acid_to_index[char] for char in tcr], dtype=torch.long)
            for tcr in tcrs if tcr
        ]
        ...
        for i, indices in enumerate(tcr_indices):
            if indices.numel() > 0:
                ...
                rep[i] = torch.zeros(embedding_tensor.shape[1], dtype=torch.float32, device=device)
